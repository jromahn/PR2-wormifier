...
######
# 1.) Taxonomy assignment in four sequential steps
### a) via known Aphia ID of the species
### b) via known Aphia ID of a species of the same genus
### c) check genus again on WoRMS
### d) check ambiguous/duplicate genus names 
# 2.) Algaebase cross-check
# 3.) Final assembly
################################################################################
...
final_path <- file.path(path_to_output,"10_FINAL_results")
...
sequences <- read.table(file.path(output_path,"8.1_Overview_Sequences_ALL.csv"), header = TRUE, sep=",")
...
################################################################################
# get ncbi taxonomy
loc_ncbi <- c("names.dmp", "nodes.dmp")
sqlFile <- 'accessionTaxa.sql'
read.accession2taxid(list.files(loc_ncbi),sqlFile,indexTaxa=TRUE)
...
sequences %>% filter(Database =="NCBI") %>% nrow()
sequences %>% filter(Database =="NCBI") %>% pull(Clean_Name) %>% unique() %>% length()
...
tax_A %>% filter(!is.na(ncbi.taxonomy)) %>% nrow

# create new column for the final taxonomy
tax_C <- tax_B %>%
  select(-Worms_record) %>%
  mutate(Kingdom = NA) %>%
  mutate(Phylum = NA) %>%
...
saveRDS(tax_1C, file.path(workspace_path, "9.1_tax_1C.RDS"))
#tax_1C <- readRDS(file.path(workspace_path,"9.1_tax_1C.RDS"))

## fill out ranks with downloaded taxonomy
tax_1D <- fill_taxonomy(tax_1C) %>%
  rowwise() %>%
...
saveRDS(tax_1E_FINAL, file.path(workspace_path, "9.1_tax_1E_FINAL.RDS"))

#stats
tax_1_empty <- tax_1E_FINAL %>% filter(is.na(Kingdom)) %>% nrow()
tax_1_known <-tax_1E_FINAL %>% filter(!is.na(Kingdom)) %>% nrow()
print(paste0("Entries with known taxonomy: ",tax_1_known, "; ", "Entries with unknown taxonomy: ",tax_1_empty ))
...
saveRDS(pr2_multi, file.path(workspace_path, "9.1_pr2_multi_genera.RDS"))
pr2_multi <- readRDS(file.path(workspace_path,"9.1_pr2_multi_genera.RDS"))
...
saveRDS(tax_2C, file.path(workspace_path, "9.1_tax_2C.RDS"))
#tax_2C <- readRDS(file.path(workspace_path,"9.1_tax_2C.RDS"))
...
saveRDS(tax_2D_FINAL, file.path(workspace_path, "9.1_tax_2D_FINAL.RDS"))

#stats
tax_2_empty <- tax_2D_FINAL %>% filter(is.na(Kingdom)) %>% nrow()
tax_2_known <-tax_2D_FINAL %>% filter(!is.na(Kingdom)) %>% nrow()
print(paste0("Entries with known taxonomy: ",tax_2_known, "; ", "Entries with unknown taxonomy: ",tax_2_empty ))


# ──────────────────────────────────────────────────────────────────
########### Step 3

#Sequences that do not have an AphiaID; Search on genus level for aphia ID
...
### ---------------------------------------- #### 
##Check several times worms for possible entries
...
saveRDS(tax_3D, file.path(workspace_path,"9.2_tax_3D.RDS"))
#tax_3D <- readRDS(file.path(workspace_path,"9.2_tax_3D.RDS"))
...
saveRDS(tax_3F, file.path(workspace_path,"9.3_tax_3F.RDS"))
#tax_3F <- readRDS(file.path(workspace_path,"9.3_tax_3F.RDS"))

#check success
test1 <- tax_3D %>% 
  filter(!is.na(as.vector(Worms_record)))
test2 <- tax_3E %>%
  filter(!is.na(as.vector(Worms_record)))
test3 <- tax_3F %>%
  filter(!is.na(as.vector(Worms_record)))
...
### ---------------------------------------- #### 
### Cleaning of the entries 
#1.) remove entries without worms entry ( tooo many)
tax_3G <- tax_3F %>%
  filter(!is.na(Worms_record)) %>%
  rowwise() %>%
  mutate(n_record = NROW(Worms_record)) %>%
  ungroup()


## 2.) get ncbi taxonomy and just keep if worms entry match expected taxonomy
...
tax_3H <- tax_3G %>%
  ...
  rowwise()%>%
  mutate(ncbiID =accessionToTaxa(paste0(accession,".1"), 'accessionTaxa.sql'))%>%
  mutate(tax_NCBI = list(getTaxonomy(ncbiID, 'accessionTaxa.sql')))%>%
  #continued
  mutate(superkingdom = tax_NCBI[1, "domain"],
         phylum       = tax_NCBI[1, "phylum"],
         class        = tax_NCBI[1, "class"],
         order        = tax_NCBI[1, "order"],
         family       = tax_NCBI[1, "family"],
         genus        = tax_NCBI[1, "genus"],
         species      = tax_NCBI[1, "species"])%>%
  select(-tax_NCBI, -ncbiID) %>%
...
# 3.) extract taxonomy of ncbi downloaded species from pr2
...
saveRDS(tax_3K, file.path(workspace_path,"9.3_tax_3K.RDS"))
#tax_3K <- readRDS(file.path(workspace_path,"9.3_tax_3K.RDS"))

## replace unknown taxonomy levels with _X
tax_3L <- fill_taxonomy(tax_3K) 
...
saveRDS(tax_3M_FINAL, file.path(workspace_path,"9.4_tax_3FINAL.RDS"))
#tax_3FINAL <- readRDS(file.path(workspace_path,"9.4_tax_3FINAL.RDS"))


#stats
tax_3_empty <- tax_3M_FINAL %>% filter(is.na(Kingdom)) %>% nrow()
tax_3_known <-tax_3M_FINAL %>% filter(!is.na(Kingdom)) %>% nrow()
print(paste0("Entries with known taxonomy: ",tax_3_known, "; ", "Entries with unknown taxonomy: ",tax_3_empty ))

  
# ──────────────────────────────────────────────────────────────────
############# Step 4 - Sequences with double names
print("Step 4 - Check again Sequences with double names")
...
tax_4G_FINAL <-   fill_taxonomy(tax_4G)
  
saveRDS(tax_4G_FINAL, file.path(workspace_path,"9.3_tax_4G_FINAL.RDS"))
save.image(file.path(workspace_path,"9.7_Workspace_before_algaebase.RDS"))
...
saveRDS(tax_6D.list, file= file.path(workspace_path, "9.6_tax_6D_algaebase.RDS"))
#tax_6D.list <- readRDS( file= file.path(workspace_path, "9.6_tax_6D_algaebase.RDS"))
...
saveRDS(tax_6F.list, file= file.path(workspace_path, "9.6_tax_6F_algaebase_known.RDS"))
#tax_6F.list <- readRDS( file= file.path(workspace_path, "9.6_tax_6F_algaebase_known.RDS"))
...
saveRDS(tax_6H, file= file.path(workspace_path, "9.6_tax_6H_algaebase_known.RDS"))
...
tax_6J_non <- tax_6H_avd %>% full_join( tax_6Inon, by = c("taxon", "Clean_Genus")) %>%
...
saveRDS(tax_6J.list, file= file.path(workspace_path, "9.6_tax_6J_algaebase_known.RDS"))
# tax_6J.list <-readRDS( file= file.path(workspace_path, "9.6_tax_6J_algaebase_known.RDS"))
...
tax_6K.cleaned <- tax_6K.accepted %>% bind_rows( tax_6K.nonacc)


## merge
tax_6L <- tax_6J %>% full_join(tax_6K.cleaned, by =c("taxon","Clean_Genus"))%>%
  mutate( acc_Name = paste(acc_Genus, Clean_Epithel))
...
## filter if multiple entires exists
priority <- c("currently accepted taxonomically", "homotypic or heterotypic synonym")  # adjust as needed
tax_6N_2 <- tax_6N %>%
  mutate(status_rank = match(`dwc:taxonomicStatus`, priority)) %>%
  group_by(Clean_Genus) %>%
  slice_min(status_rank, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  select(-status_rank)

tax_6O <- tax_6N_2 %>% full_join(tax_6M.pr2, by=c("taxon", "Clean_Genus"))
...
#final
tax_6O.cleaned <- tax_6O.accepted %>% bind_rows( tax_6O.nonacc)%>% 
  mutate(pr2_division =NULL, pr2_subdivision=NULL, pr2_class =NULL)%>%
  mutate(acc_Name= paste(acc_Genus, Clean_Epithel))
...
tax_6Q <- filter_algaebase_for_algae(tax_6P) %>%
  ...
  filter(is.na(data_ncbi.taxonomy) | 
           str_detect(data_ncbi.taxonomy, fixed(Class)))
...
write.table(tax_6R, file = file.path(final_path, "PR2_metadata_Algaebase.csv"), row.names = F, sep=",")

#####

tax_6R_FINAL <-tax_6R %>% select(all_of(colnames(tax_1E_FINAL))) 
saveRDS(tax_6R_FINAL, file.path(workspace_path, "9.6_tax_6R_FINAL.RDS"))
...
save.image(file.path(workspace_path,"9.6_Workspace.RData"))
#load(file.path(workspace_path,"9.6_Workspace.RData"))
...
write.table(sequences_FINAL,file.path(final_path, "9.6_Overview_Sequences_FINAL.csv"), col.names = TRUE, row.names = FALSE, sep=",")
save.image(file.path(workspace_path,"9.7_Workspace.RData"))

#load(file.path(workspace_path,"9.7_Workspace.RData"))

end_time <- Sys.time()
print(paste0("Analysis started: ", start_time))
print(paste0("Analysis ended: ", end_time))

duration <- end_time - start_time
print(paste0("Analysis duration: ", round(duration,1), "hours"))

## Check for differences
diff <- sequences_FINAL %>%filter( Clean_Name != Acc_Name) %>%select(Clean_Name, Acc_Name, species) %>% unique()
diff_filtered <- diff %>% filter( !mapply(grepl, "sp\\.", species))
