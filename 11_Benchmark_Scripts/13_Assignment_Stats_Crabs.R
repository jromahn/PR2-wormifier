...
identif <- identify_classified_reads(results_1_clean, counttable, Class, "Ciliophora", "v1")
stats <- identif[[1]]
classified_1_Cil_fam <- identif[[2]]
classified_1_Cil_gen <- identif[[3]]
classified_1_Cil_spec <- identif[[4]]
classified_1_Cil <- identif[[5]]
...
#Classified Ciliates (all, species level, genus level)
identif <- identify_classified_reads(results_2_clean,counttable, Class, "Ciliophora", "v2")
stats <- identif[[1]]
classified_2_Cil_fam <- identif[[2]]
classified_2_Cil_gen <- identif[[3]]
classified_2_Cil_spec <- identif[[4]]
classified_2_Cil <- identif[[5]]
assignment_stats <- rbind(assignment_stats,stats)
...
#Classified Ciliates (all, species level, genus level)
identif <- identify_classified_reads(results_3_clean,counttable, Phylum, "Ciliophora", "v3")
stats <- identif[[1]]
classified_3_Cil_fam <- identif[[2]]
classified_3_Cil_gen <- identif[[3]]
classified_3_Cil_spec <- identif[[4]]
classified_3_Cil <- identif[[5]]
assignment_stats <- rbind(assignment_stats,stats)
...
#Classified Ciliates (all, species level, genus level)
identif <- identify_classified_reads(results_4_clean, counttable,Phylum, "Ciliophora", "v4")
stats <- identif[[1]]
classified_4_Cil_fam <- identif[[2]]
classified_4_Cil_gen <- identif[[3]]
classified_4_Cil_spec <- identif[[4]]
classified_4_Cil <- identif[[5]]
assignment_stats <- rbind(assignment_stats,stats)
...
write.table(stats_family, file=file.path(output_path_tab,"13_Ciliate_family_list_RefDBcomparison.tsv"), sep="\t", row.names = F)
...
baltic <-nrow(baltic_4_Cil_gen)/nrow(classified_4_Cil_gen)*100
print(baltic)
...
compare_genus_level(classified_1_Cil_gen, classified_2_Cil_gen, classified_3_Cil_gen, classified_4_Cil_gen,baltic_ciliates, "Ciliate", output_path_tab)
...
ggsave(file.path(output_path, "13_Figure_bootstrap_changes_Ciliate.jpg"), dpi = 300, width=8, height = 6)
saveRDS(plot_data, file.path(output_path,"13_plotData_bootstrap_stats_Euka02_Cil.RDS"))
...
write.table(assignment_stats, file=file.path(output_path_tab,"13_SUMMARY_Read_ASV_statistics_Ciliates.tsv"), sep="\t", row.names = F)
write.table(comparison_ASVs_id, file=file.path(output_path_tab,"13_Ciliate_Analysis_RefDB_comparison__assigned.tsv"), sep="\t", row.names = F)
write.table(comparison_ASV_Spec_id, file=file.path(output_path_tab,"13_Ciliate_Analysis_RefDB_comparison__species.tsv"), sep="\t", row.names = F)
write.table(comparison_ASV_Gen_id, file=file.path(output_path_tab,"13_Ciliate_Analysis_RefDB_comparison__genus.tsv"), sep="\t", row.names = F)
...
plot1 <- ggplot(stats_classified, aes(fill = tax_unit, y = num_classified, x = database)) +
  ...
  theme(axis.title.x = element_text(vjust = -2), axis.title.y = element_text(vjust = 2), title = element_text(vjust = 2)) 
print(plot1)
ggsave(file.path(output_path, "13_Figure_stats_classified__identifiedASVs_Ciliate.jpg"), dpi = 300, width=9, height = 5)
...
plot2 <- ggplot(stats_classified_tax, aes(fill = tax_unit, y = num_classified_tax, x = database)) +
  ...
  theme_light() +
  scale_fill_manual(values=color_ciliate, 
                    labels=c("genus"="Genus", "species"="Species"))+
  theme(axis.title.x = element_text(vjust = -2), axis.title.y = element_text(vjust = 2), title = element_text(vjust = 2)) 
print(plot2)
ggsave(file.path(output_path, "13_Figure_stats_classified__identifiedTaxa_Ciliate.jpg"), dpi = 300, width=9, height = 5)
...
plot3 <- ggplot(stats_classified_reads, aes(fill = tax_unit, y = num_classified_reads, x = database)) +
  ...
  theme(axis.title.x = element_text(vjust = -2), axis.title.y = element_text(vjust = 2), title = element_text(vjust = 2)) 
print(plot3)
ggsave(file.path(output_path, "13_Figure_stats_classified__identifiedReads_Ciliate.jpg"), dpi = 300, width=9, height = 5)
...
ggsave(file.path(output_path, "13_Figure_stats_classified__all_Ciliates.jpg"), dpi = 300, width=9, height = 15)
################################################################################
################################################################################

#Save cleaned results for Treemap script
saveRDS(results_1_clean, file.path(output_path,"13__Results_DB1_Cil.RDS"))
saveRDS(results_2_clean, file.path(output_path,"13__Results_DB2_Cil.RDS"))
saveRDS(results_3_clean, file.path(output_path,"13__Results_DB3_Cil.RDS"))
saveRDS(results_4_clean, file.path(output_path,"13__Results_DB4_Cil.RDS"))

################################################################################
#keep stats for plotting

stats_classified <- stats_classified %>% 
  mutate(organism= "Ciliophora", dataset= "TAREuk", method="ASV")%>%
  rename(values= num_classified)

stats_classified_tax<- stats_classified_tax %>% 
  mutate(organism= "Ciliophora", dataset= "TAREuk", method="Taxa")%>%
  rename(values= num_classified_tax)

stats_classified_reads <- stats_classified_reads%>% 
  mutate(organism= "Ciliophora", dataset= "TAREuk", method="Reads")%>%
...
saveRDS(stats_classified_combined, file.path(output_path,"13__combined_stats_Euka02_Cil.RDS"))
