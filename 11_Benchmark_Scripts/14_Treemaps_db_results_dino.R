...
################################################################################
#Run after '13_Assignment_Stats_CRABS_dino.R'
#TAxon: Dinophyceae
################################################################################
rm(list = ls())
...
results_db1 <- readRDS(file.path(input_path,"13__Results_DB1_Dino.RDS"))
results_db2 <- readRDS(file.path(input_path,"13__Results_DB2_Dino.RDS"))
results_db3 <- readRDS(file.path(input_path,"13__Results_DB3_Dino.RDS"))
results_db4 <- readRDS(file.path(input_path,"13__Results_DB4_Dino.RDS"))
...
results_db1_fam <- filter_taxon(results_db1, counttable, Family, Subclass,"Dinophyceae")
results_db4_fam <- filter_taxon(results_db4, counttable, Family, Class,"Dinophyceae")

#Prepare results so that they only contain ciliates, assigned to at least genus
#level, not removed by cleaning. Number of rows indicate number of reads
results_db1 <- filter_taxon(results_db1, counttable, Genus, Subclass,"Dinophyceae")
results_db2 <- filter_taxon(results_db2, counttable, Genus, Subclass,"Dinophyceae")
results_db3 <- filter_taxon(results_db3, counttable, Genus, Class,"Dinophyceae")
results_db4 <- filter_taxon(results_db4, counttable, Genus, Class,"Dinophyceae")

## treemap stats --> readcount count
A_stats <- pr2_treemap_stats(db_4 %>% filter(subdivision == "Dinophyceae"), class, genus)

#to family level
B_stats_fam <- pr2_treemap_stats(results_db1_fam, Order, Family)
C_stats_fam <- pr2_treemap_stats(results_db4_fam, Order, Family)

#to genus level
B_stats_gen <- pr2_treemap_stats(results_db1, Family, Genus)
C_stats_gen <- pr2_treemap_stats(results_db4, Family, Genus)
# to species level
B_stats_spec <- pr2_treemap_stats(results_db1, Genus, Species)
C_stats_spec <- pr2_treemap_stats(results_db4, Genus, Species)

######------------- treemap stats --> ASV count
## to family 
ASV_family_db1 <- summarize_stats(results_db1, Family, Subclass,"Dinophyceae")
ASV_family_db4 <- summarize_stats(results_db4, Family, Class,"Dinophyceae" )

## to genus 
ASV_genus_db1 <- summarize_stats(results_db1, Genus, Subclass,"Dinophyceae" )
ASV_genus_db4 <- summarize_stats(results_db4, Genus, Class,"Dinophyceae" )

## to species 
ASV_species_db1 <- summarize_stats(results_db1, Species, Subclass,"Dinophyceae"  )
ASV_species_db4 <- summarize_stats(results_db4, Species, Class,"Dinophyceae"  )
...
write.table(stats_assignment, file=file.path(output_path,"14_RefDB_summary_Dinophyceae.tsv"), sep="\t", row.names = F)
...
write.table(stats_family, file=file.path(output_path_tab,"14_RefDB_comparison__reads_family_Dinophyceae.tsv"), sep="\t", row.names = F)
write.table(stats_genus, file=file.path(output_path_tab,"14_RefDB_comparison__reads_genus_Dinophyceae.tsv"), sep="\t", row.names = F)
write.table(stats_species, file=file.path(output_path_tab,"14_RefDB_comparison__reads_species_Dinophyceae.tsv"), sep="\t", row.names = F)

rm(B_stats_fam, C_stats_fam,B_stats_gen, C_stats_gen, B_stats_spec, C_stats_spec )

####################################### Plot ####################################### 
#Treemaps of the database versions
pr2_treemap_db(db_1 %>% filter(class == "Dinophyceae"), order, genus)
pr2_treemap_db(db_2, subdivision, class)
pr2_treemap_db(db_3, domain, class)
A <- pr2_treemap_db(db_4 %>% filter(subdivision == "Dinophyceae"), order, genus)   +
  labs(title=paste(primer,"Dinophyceae", sep=" - "))+
  theme(plot.title= element_text(face="bold", size=28))



#Treemaps of the ciliate results
B <- pr2_treemap_results(bind_rows(results_db1, distinct(results_db4)), Genus, Species) 
pr2_treemap_results(results_db2, Genus, Species)
pr2_treemap_results(results_db3, Genus, Species)
C <- pr2_treemap_results(bind_rows(results_db4, distinct(results_db1)), Genus, Species)
...
distinct(bind_rows(results_db4, results_db1))


length_col <- db_4%>% filter(subdivision == "Dinophyceae") %>% count(order) %>% nrow()
A <- A+ scale_fill_manual(values = colorRampPalette(ghibli_palettes$MarnieMedium1)(length_col))+
  labs(title=paste(primer,"Dinophyceae", sep=" - "))+
  theme(plot.title= element_text(face="bold", size=28))
...
ggarrange(A + plot_arrange, # Adjust left margin (l = 50)
          ...
          font.label = list(size = 18), hjust = -0.7)
ggsave(file.path(output_path, "14_Figure_stats_treemap_Dinophyceae.jpg"), dpi = 300, width=12, height = 8)
