...
saveRDS(spec_A,file = file.path(workspace_path, "2.1_spec_A.RDS"))
#spec_A <- readRDS(file.path(workspace_path,"2.1_spec_A.RDS"))
...
spec_B <- spec_A %>%
  ...
  mutate(
    worms_record = list(
      worms_record %>%
        filter(grepl(taxon, paste(kingdom, phylum, class, order, family, sep = ";"), fixed = TRUE) | is.na(taxon)) )) %>%
...
saveRDS(spec_D,file = file.path(workspace_path, "2.1_spec_D.RDS"))
spec_D <- readRDS(file.path(workspace_path,"2.1_spec_D.RDS"))

taxonomy <- spec_D[,c("NR", "genus", "species", "acc_Name","taxon", "taxonomy")]
spec_D$taxonomy <- NULL

write.table(spec_D, file.path(workspace_path,"2.2_spec_D.csv"), row.names = FALSE,sep=",")
...
saveRDS(spec_E, file.path(workspace_path,"2.3_spec_E.RDS"))
#spec_E <- readRDS(file.path(workspace_path,"2.3_spec_E.RDS"))

print("Start spec_FINAL")
spec_FINAL <- spec_E %>%
  rowwise() %>%
  mutate(scientificname=data$species)%>% # mod by JR
  unnest(cols = new_syn) %>%
...
write.table( spec_FINAL, file.path(output_path,"2.4_F_species_FINAL.csv"),sep=",",row.names = F)
...
pattern <- "Chlorophyta|Rhodophyta|Charophyta|Ochrophyta|Myzozoa|Cryptophyta|Haptophyta|Cyanobacteria|Tracheophyta|Bacillariophyta|Euglenozoa"
species_phyta <- taxonomy %>%
  filter(grepl(pattern, taxonomy) | grepl(pattern, taxon))
...
search <- search[,c("request","taxon", "Full_search", "NR")]%>% unique()
algaebase.species.df <- algaebase.species.df %>% full_join( search, by=c("request", "taxon"),relationship = "many-to-many")

#cleaning results ────────────────────────────────────────────────────────────────
algaebase.species.df <-algaebase_species_clean_up(algaebase.species.df)
write.table(algaebase.species.df, file.path( output_path,"2.8_F_Algaebase_specieslist.csv"), row.names = FALSE,sep=",")

## combine with specFINAL ────────────────────────────────────────────────────────────────
print("Start spec_FINAL_new")
spec_FINAL_algaebase <- algaebase.species.df[,c("NR","acc_Name")]
spec_FINAL_new <- spec_FINAL%>% full_join( spec_FINAL_algaebase, by = "NR")
...
spec_FINAL_new <- spec_FINAL_new %>%bind_rows(new_synoyms)%>% 
    arrange(NR) %>% mutate(acc_Name.x=NULL, acc_Name.y = NULL)

spec_FINAL_new <- spec_FINAL_new %>%  mutate(across(where(is.character), strip_html))

write.table( spec_FINAL_new, file.path(output_path,"2.9_F_species_FINAL_withAlgaebase.tsv"), row.names = FALSE,sep="\t")
saveRDS(spec_FINAL_new,file.path(output_path, "2.9_F_species_FINAL_withAlgaebase.RDS") )


save.image(file.path(workspace_path, "2.10_Workspace.RData"))
load(file.path(workspace_path, "2.10_Workspace.RData"))
