...
# 4.) 18S filtering 
# 5.) WoRMS validation
# 6.) Algaebase cross-check
####################

#input
input_path_ncbi <-  file.path(path_to_output,"02_NCBI_20_Apr_2026_results") # results of a script

# output from python script 05_Check_NCBI_with_biopython__advanced.py
ncbi_date <- gsub(".*02_NCBI_(.*)_results","\\1",input_path_ncbi)
ncbi_downloaded_table <- paste0("05_Missing_PR2_Downloaded_species_NCBI_info__", ncbi_date,".tsv") 
...
colnames(spec_new) <- c("search", "species", "accession_number", "ncbi.taxonomy", "file")
...
ncbi_files <- unique(spec_X$file)
ncbi_seq <- data.frame()


for (i in c(1:length(ncbi_files))) {
  # Reads the file as a character vector
  content <- readLines(ncbi_files[i], n = 1) 
  
  if (nchar(content) > 1) {
    # 1. Read the file
    fasta <- read.fasta(file = file.path(ncbi_files[i]), seqtype = "DNA", as.string = TRUE)
    
    # 2. Convert to a Dataframe
    df <- data.frame(
      fasta =ncbi_files[i],
      header = sapply(fasta, function(x) attr(x, "Annot")),
      sequence = unlist(fasta),
      row.names = NULL)%>%
      mutate(accession = gsub(">([a-zA-Z0-9_]+).*","\\1", header))
    
    ncbi_seq <- ncbi_seq %>% bind_rows(df)
  }
}
...
spec_Y <- spec_X %>%
  unnest(cols = data) %>%
  #unnest(cols = header) %>%
  right_join( ncbi_seq , by=c("accession_number"="accession", "file"="fasta") )%>%
  rowwise() %>%
  mutate(length = str_length(sequence))%>%
  ungroup() %>%
  #dplyr::rename(header = header) %>%
...
spec_Z <- spec_Y %>%
  ...
  filter(!str_detect(header, "PREDICTED")) %>%
...
saveRDS(spec_D, file.path(workspace_path,"6.2_spec_D.RDS"))
spec_D <- readRDS(file.path(workspace_path,"6.2_spec_D.RDS"))

save.image(file.path(workspace_path, "6.3_Workspace.RData"))
...
missing_gen <- unique(filtered_dataframe[,c("genus", "taxon")])%>%
  group_by(genus) %>%
  filter(!(is.na(taxon) & any(!is.na(taxon)))) %>%
  ungroup()

missing_gen$genus[duplicated(missing_gen$genus)]

spec_D2 <- spec_D %>% mutate(genus = gsub("^([\\w+\\-]+).*", "\\1", Clean_Name, perl=T))%>% unique()
spec_D3 <- spec_D2 %>% full_join( missing_gen, by ="genus",
                                  relationship = "many-to-many") #new


## remove entries with non matching taxonomy  ────────────────────────────────────────────────────────────────
spec_E <- spec_D3 %>% 
  rowwise() %>%
  mutate(n_records = sum(!is.na(worms_record[[1]]))) %>%
  filter(n_records > 0) %>%
  # check if taxonomy fits
  mutate(worms_record = list(subset(worms_record, worms_record$kingdom == taxon |   
                                                  worms_record$phylum == taxon |
                                                  worms_record$class == taxon |
                                                  worms_record$order == taxon |
                                                  worms_record$family == taxon | 
                                                  is.na(worms_record$phylum)| 
                                                  is.na(taxon)))) %>%
...
spec_F <- spec_E2 %>%
  ...
  rowwise() %>%
  # calculcate replicate number for species
...
spec_F_FINAL <- spec_F %>% 
  rowwise() %>% 
  filter(is.na(taxon)|grepl(taxon, ncbi.taxonomy))%>%
  ungroup()
spec_F_removed <- spec_F %>% 
  rowwise() %>% 
  filter(!grepl(taxon, ncbi.taxonomy))%>%
  ungroup()


## double check removed if taxon is just not known by ncbi or names differently 
spec_F_FINAL2 <- data.frame()

#  lookup/replacement table  ────────────────────────────────────────────────────────────────
replacements <- data.frame(
  from = c("Bacillariophyceae", "Charophyta",    "Cyanobacteria",    "Chlorophyta",    "Cryptophyta"), #worms
  to   = c("Bacillariophyta",   "Viridiplantae", "Cyanobacteriota",  "Chlorophyceae",  "Cryptophyceae"), #ncbi
  stringsAsFactors = FALSE
)

if (nrow(spec_F_removed) > 0) {
  
  spec_F_removed <- spec_F_removed %>%
    mutate(taxon2 = reduce2(replacements$from, replacements$to, .init = taxon,
                            function(x, pattern, replacement) gsub(pattern, replacement, x)))
  
  spec_F_NOTremoved <- spec_F_removed %>%
    filter(mapply(function(p, s) grepl(p, s, fixed = TRUE), taxon2, ncbi.taxonomy)) %>%
    select(-taxon2)
  
  spec_F_removed2 <- spec_F_removed %>%
    filter(!mapply(function(p, s) grepl(p, s, fixed = TRUE), taxon2, ncbi.taxonomy))
  
  spec_F_FINAL2 <- spec_F_FINAL %>% bind_rows(spec_F_NOTremoved)
...
write.table(spec_F_FINAL2, file.path(output_path,"6.4_Species_NCBI.csv"), sep=",", row.names = F)
save.image(file.path(workspace_path, "6.5_Workspace.RData"))
