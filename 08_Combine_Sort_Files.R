...
################################################################################
## Aim: Structure and clean NCBI results
## Function: Merges cleaned NCBI and PR² sequences, assigns metadata, and organizes NCBI files to avoid duplication.
######
# 1.) AphiaID fill-in
# 2.) Deduplication & renumbering 
# 3.) NCBI file reorganization 
################################################################################
...
spec_ncbi <- read.table(file.path(output_path, "6.4_Species_NCBI.csv"), header = TRUE, sep=",")
rest_pr2 <- read.table(file.path(output_path, "7.1_Overview_PR2_Rest.csv"), header = TRUE, sep=",") %>% mutate(file = NA)

################################################################################
##output
out_path_seq <- file.path(path_to_output, "PR2_Sequences/NCBI")
...
for (i in c(1:nrow(overview_cleaned))) {
  if (overview_cleaned[i,]$Database == "NCBI") {
    ...
    #changed so the version of an ancession number is removed and ignored for the comparison
    seq <- seqs[gsub("\\.\\d+", "",names(seqs)) == overview_cleaned[i,]$genbank_accession]
...
write.table(overview_FINAL, file.path(output_path, "8.1_Overview_Sequences_ALL.csv"), sep=",", col.names = TRUE, row.names = FALSE)

save.image(file.path(workspace_path,"8.2_Workspace.RData"))
